...
export function ServiceLandingPage({data}: Props) {
  ...
  return (
    <div className="site-chrome">
      ...
      <section className="wrap" id="why-us"><h2>{questionHeading(service.whyHeading)}</h2><p className="lede narrow">{service.whyLede}</p><div className="why-grid">{service.whyItems?.map((item) => <details className="why-item" open key={item._key || item.title}><summary><span>{questionHeading(item.title)}</span><span className="why-plus">+</span></summary><p className="why-body">{item.body}</p></details>)}</div></section>

      <section className="section-tint" id="working-in-area"><div className="wrap"><h2>{questionHeading(`Working in ${area.name}`)}</h2><p className="lede narrow">{area.workingLede}</p><div className="photo-grid">{page.workingPhotos?.map((photo, index) => <div className="ph" key={photo._key || index} style={imageStyle(photo)} role="img" aria-label={photo.alt || `${service.name} work in ${area.name} ${index + 1}`}>{!imageUrl(photo) && <EmptyImageIcon />}</div>)}</div></div></section>

      <section className="wrap" id="areas"><h2>{questionHeading(area.areasHeading)}</h2><p className="lede narrow">{area.areasLede}</p><div className="coverage-stack">{coverageMapFirst ? <>{coverageMap}{coverageAreas}</> : <>{coverageAreas}{coverageMap}</>}</div>{area.areasNote && <p className="small muted coverage-note">{area.areasNote}</p>}</section>
      ...
      <section className="wrap"><div className="cta-final"><h2>{questionHeading(`${service.ctaHeading} in ${area.name}?`)}</h2><p>{service.ctaBody}</p><div className="btn-row centered"><a className="btn btn-primary" href={`tel:${settings.phoneE164}`}>Call {settings.phoneDisplay}</a><a className="btn btn-secondary" href="#quote">{service.secondaryCta}</a></div></div></section>
